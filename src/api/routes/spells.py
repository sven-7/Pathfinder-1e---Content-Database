...
async def list_spells(
    ...
    if class_name and level is not None:
        rows = db._many(
            """SELECT s.id, s.name, s.school, s.subschool, s.descriptors,
        ...
        rows = db._many(
            """SELECT DISTINCT s.id, s.name, s.school, s.subschool, s.descriptors,
                      s.casting_time, s.components, s.range, s.duration,
                      s.saving_throw, s.description, s.url, scl.level as spell_level
               FROM spells s
               JOIN spell_class_levels scl ON scl.spell_id = s.id
        ...
        rows = db._many(
            """SELECT id, name, school, subschool, descriptors,
